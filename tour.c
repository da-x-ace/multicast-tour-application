...
void proc_v4(char *ptr, ssize_t len, struct msghdr *msg, struct timeval *tvrecv)
{
	...
	if (icmp->icmp_type == ICMP_ECHOREPLY) {
		...
		if((sin->sin_addr.s_addr == myIP.s_addr) || (inVMList(sin->sin_addr.s_addr) == 0))
		{
			return;
		}
...
void send_v4(void)
{
	...
	for(i=0; i<indexPing; i++)
	{
		...
		destaddr.sin_addr.s_addr = firstTime[i];
...
void ping_v4(char* vmToPing)
{
	...
	memset(&ipOfVM, 0, sizeof(struct in_addr));	
	ipOfVM = getIPAddr(vmToPing);

	if(ipOfVM.s_addr == myIP.s_addr)
	{
		//Will not ping my own VM
		return;
	}
